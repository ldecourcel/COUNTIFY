class InvoicesApi
  ...
  def call(file)
    ...
    @invoice.total_amount_cents = (result["document"]["inference"]["pages"][0]["prediction"]["total_incl"]["value"]*100).to_i
    ...
    if result["document"]["inference"]["pages"][0]["prediction"]["taxes"] != []
      @invoice.tax_amount_cents = (result["document"]["inference"]["pages"][0]["prediction"]["taxes"][0]["value"]*100).to_i
      @invoice.vta = result["document"]["inference"]["pages"][0]["prediction"]["taxes"][0]["rate"]
    else
      @invoice.tax_amount_cents = @invoice.total_amount - @invoice.net_amount unless @invoice.total_amount.nil? || @invoice.net_amount.nil?
    end
